...
impl BlockDiffusionDraftHead {
    pub fn from_weights(
        ...
        gpu: &dyn GpuBackend,
        max_seq_len: usize,
        // Proposer TP: `Some((rank, world))` shards the drafter column-parallel
        // across the target's TP group (see `BlockDiffusionDraftHead::tp`).
        ...
        let mut intermediate_size = weights.config.intermediate_size;
        let mut num_q_heads = weights.config.num_attention_heads;
        let mut num_kv_heads = weights.config.num_key_value_heads;
        ...
        if let Some((rank, world)) = tp {
            ...
            anyhow::ensure!(
                num_q_heads.is_multiple_of(world)
                    && num_kv_heads.is_multiple_of(world)
                    && intermediate_size.is_multiple_of(world)
                    && hidden_size.is_multiple_of(world)
                    && vocab_size.is_multiple_of(world),
            ...
            let h_l = h / world;
            // Row-major `[N, K]` weights: rank r's contiguous row slice is a byte offset.
            // N-side shards (output columns): q/k/v/gate/up take rows of N = q/kv/inter;
        ...
        // Proposer TP half-width staging: the largest local output any gather
        // moves — the fc projection over up to `ctx_window` ctx rows (`h / w`
        ...
        let scratch = DflashScratch {
            ...
            tp_local: if let Some((_, w)) = tp {
                gpu.alloc(tp_half_bytes(w))?
            } else {
                DevicePtr(0)
            },
            tp_recv: if let Some((_, w)) = tp {
                gpu.alloc(tp_half_bytes(w))?
            } else {
                DevicePtr(0)
            },
            tp_bytes: tp.map(|(_, w)| tp_half_bytes(w)).unwrap_or(0),
            tp_full: if let Some((_, w)) = tp {
                gpu.alloc(nb * g * q_dim.max(intermediate_size) * w * bf16)?
            } else {
                DevicePtr(0)
            },
        };
            ...
            && tp_attn
        {
            ...
            weights.fc.weight = weights
                .fc
                .weight
                .offset(rank * (hidden_size / world) * t * 2);
        }
        let mut head = Self {
            num_layers,
            hidden_size,
