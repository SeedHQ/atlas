...
impl ModelConfig {
    ...
    /// (`ATLAS_GLM53_PREFIX_CACHE_UNPROVEN`) that opens the prefix-cache arm and
    ...
    ///
    /// 🧪 `ATLAS_GLM53_PREFIX_CACHE_UNPROVEN` (PRESENCE, house convention — `=0`
    ...
    /// The `ATLAS_GLM53_PREFIX_CACHE_UNPROVEN` validation switch (PRESENCE).
    fn glm53_prefix_cache_validation_env() -> bool {
        std::env::var_os("ATLAS_GLM53_PREFIX_CACHE_UNPROVEN").is_some()
    }
